...
`millfolio-ws`, and the UI from `headgate/web` to `app/web`. Staged so the shipping
vault path is never broken.

## 5a — app release bundle  ✅ (this step)

`app-zip.yml` builds `millfolio-server` + `millfolio-ws` (compile gate) and the web
UI, then `scripts/package-app.sh` packages **`millfolio-app.zip`** = `src/ws_server.mojo`
(+ `server.mojo`) + `web/dist/`. Attached to a Release on tags. CI-validated.

## 5b — CLI install + launch  (next; Swift, locally verifiable)

In `millfolio/cli` `Bootstrapper.swift`:

- `installAppServer()` — download `millfolio-app.zip` from `millfolio/app`'s
  `releases/latest`, unzip under `~/Library/Application Support/Millfolio/app/`,
  ...
  → `app/build/millfolio-ws`. (Reuses `headgate-mojo`'s prefix + flare shims, so no
  new toolchain/shims.)
- launch: replace `writeMillfolioWebScript` (which execs headgate's `serve-web.sh`)
  with one that `cd`s to `app/` (so `./web/dist` resolves), sets `CONDA_PREFIX=
  headgate-mojo`, `HEADGATE_MILLFOLIO` + `MILLFOLIO_VAULT`/loopback URLs, and execs
  `./build/millfolio-ws`.
...
Release cli (and tag `app` to publish `millfolio-app.zip`).
