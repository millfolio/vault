# enclave document mode — design

**Goal: let users drop a folder of their own files — PDFs, Word docs, text,
emails — and ask questions about them, with the same guarantee enclave gives
today: a frontier model writes the code, your content never leaves the machine.**

Today enclave v1 is built end to end around a **single CSV table**
...
enclave's confidentiality trick (README): the model is allowed to see the
...
Documents tempt a task enclave **cannot** safely serve via the remote model:
...
enclave should detect/declare this boundary and route accordingly, rather than
...
Output: a normalized text cache (e.g. `~/.config/enclave/cache/<run>/doc_0.txt`)
