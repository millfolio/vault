...
`enclave-system.md` is the system prompt the **frontier model** follows to write
...
`enclave codegen "<q>" --manifest <file>` subcommand — **no index or embedding
...
- `run_eval.sh [MODEL]` — runs each case (with `ENCLAVE_MODEL=<MODEL>`) and greps
...
`core/test/extract/transactions_test.mojo`), index it, run `enclave vault` per
