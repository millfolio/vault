...
# lint logic can't silently rot. Fully mock-driven: a fake `enclave` emits a
...
# Mock enclave. Invoked as: codegen "<question>" --manifest <path>. Emits a
...
run() { # golden_file [model] -> sets rc
  local golden="$1"
  shift
  ENCLAVE_BIN="$TMP/mockbin" EVAL_GOLDEN="$golden" \
...
ENCLAVE_BIN="$TMP/mockbin" bash "$DIR/run_eval.sh" no-such-model >/dev/null 2>&1
