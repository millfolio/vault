...
#
# Drives `enclave codegen` on a SYNTHETIC manifest (codegen only ever sees the
...
# to codegen via ENCLAVE_MODEL. Re-run the eval after ANY model change and after
...
# what we're evaluating). Override the binary with ENCLAVE_BIN=… (used by
...
BIN="${ENCLAVE_BIN:-$ROOT/build/enclave}"
...
if [ ! -x "$BIN" ] && [ -z "${ENCLAVE_BIN:-}" ]; then
  ...
  echo "enclave not built — building it (pixi run enclave)…" >&2
  (cd "$ROOT" && pixi run enclave) >&2 ||
    { echo "build failed; run: (cd '$ROOT' && pixi run enclave)" >&2; exit 2; }
fi
if [ ! -x "$BIN" ]; then
  echo "enclave binary not found at $BIN" >&2
  exit 2
fi
if [ -z "${ANTHROPIC_API_KEY:-}" ] && [ -z "${ENCLAVE_BIN:-}" ]; then
...
# The model under evaluation (codegen reads ENCLAVE_MODEL). Skip when a mock
# binary is driving the harness self-test (it ignores the model).
if [ -z "${ENCLAVE_BIN:-}" ]; then
  export ENCLAVE_MODEL="$MODEL"
  # CRITICAL: point codegen at the REAL system prompt. _codegen_system() resolves
  # `resources/enclave-system.md` relative to ENCLAVE_HOME/cwd; from here cwd
  ...
  export ENCLAVE_PROMPT="${ENCLAVE_PROMPT:-$DIR/../resources/enclave-system.md}"
  [ -f "$ENCLAVE_PROMPT" ] || { echo "real prompt not found at $ENCLAVE_PROMPT" >&2; exit 2; }
fi
# Codegen with a zero remote budget falls back to local-only; give it room.
export ENCLAVE_REMOTE_TOKEN_BUDGET="${ENCLAVE_REMOTE_TOKEN_BUDGET:-200000}"
...
CLEANUP_DIR=""
if [ -z "${ENCLAVE_BIN:-}" ] && [ -d "$DIR/fixtures/data" ]; then
...
# enclave's codegen HTTPS call dlopens flare's FFI libs by the RELATIVE path
