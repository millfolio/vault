"""settings — load enclave config from ~/.config/enclave/config.json + env.
...
ENCLAVE_CONFIG. Secrets (`anthropic_api_key`) are better supplied via the
...
def config_path() -> String:
    return getenv(
        "ENCLAVE_CONFIG",
        getenv("HOME", "") + "/.config/enclave/config.json",
    )
...
def load_config() -> Config:
    ...
    local_url = _env_or("ENCLAVE_LOCAL_URL", local_url^)
    local_model = _env_or("ENCLAVE_LOCAL_MODEL", local_model^)
    ...
    data_dir = _env_or("ENCLAVE_DATA", data_dir^)

    # empty data_dir -> the default under ~/.config/enclave (created/asked for at
    # first run by enclave.mojo). Kept here so the key is overridable everywhere.
    if data_dir == "":
        data_dir = getenv("HOME", "") + "/.config/enclave/data"
