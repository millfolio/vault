...
enclave's `pi-ai`-equivalent layer (PRIOR-ART.md): the one place network I/O
...
MOCK path: when ANTHROPIC_API_KEY is unset or ENCLAVE_MOCK is set, codegen returns
...
def _prompt_path() raises -> String:
    """Where to load the system prompt from. `ENCLAVE_PROMPT` (an absolute path)
    overrides; otherwise `resources/enclave-system.md` resolved under
    `ENCLAVE_HOME` (every launcher sets it — see resource_path). NEVER cwd.
    """
    var override = getenv("ENCLAVE_PROMPT", "")
    if override != "":
        return override
    return resource_path(String("resources/enclave-system.md"))


def _viewgen_system() raises -> String:
    """System prompt for the Millwright VIEW-plane editor (dashboard-spec edits).
    Loaded from `resources/millwright-viewgen-system.md` (ENCLAVE_VIEWGEN_PROMPT
    overrides; resolved by ABSOLUTE path like the codegen prompt, never cwd).
    Same fail-loud contract as `_codegen_system`."""
    var override = getenv("ENCLAVE_VIEWGEN_PROMPT", "")
...
def _codegen_system() raises -> String:
    ...
    `resources/enclave-system.md` (resolved by ABSOLUTE path — ENCLAVE_HOME /
    ENCLAVE_PROMPT, never cwd) so the agent's contract — the confidentiality rules,
