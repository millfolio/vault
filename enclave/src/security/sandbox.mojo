"""Sandbox — the CONTAINMENT half of enclave. Runs generated code in a box that
cannot phone home or escape its scope.

The boundary is PROVEN: see sandbox/enclave.sb.template + sandbox/spike.sh +
...
struct Sandbox(Movable):
    ...
    var template_path: String  # sandbox/enclave.sb.template
    ...
    def _render_profile(self, scratch_c: String) raises -> String:
        ...
        (enclave-vault.sb.template) instead — same containment, plus a localhost-
        only network allowance + the index dir read-allow."""
        var is_vault = self.policy.network == "loopback"
        var tmpl_path = _replace_all(
            self.template_path,
            String("enclave.sb.template"),
            String("enclave-vault.sb.template"),
        ...
        var path = scratch_c + (
            "/enclave-vault.sb" if is_vault else "/enclave.sb"
        )
        _write(path, rendered)
        return path

    def run(self, binary: String, args: List[String]) raises -> RunResult:
        """Run `binary args...` under sandbox-exec with the rendered enclave
    ...
    def compile(
        ...
        step is separately contained + read-scoped (enclave{,-vault}.sb.template).
        """
        var scratch_c = _canonical(self.policy.scratch_dir)
        ...
        # activation (e.g. ./build/enclave), so don't rely on `mojo` being on PATH.
