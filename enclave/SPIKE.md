...
**Question:** can enclave's code-runner confine remote-generated code so it
...
`sandbox/enclave.sb.template`; the proof is `sandbox/spike.sh` (run it with
`pixi run spike`, or directly — it needs no Mojo toolchain).

## Result

```
enclave sandbox spike
...
2. **Importing the system baseline does.** `(import ".../bsd.sb")` sets up the
   ...
   `/System`. Then layer the enclave denials: `network*`, `file-write*` (except
