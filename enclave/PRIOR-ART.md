# Prior art: agent harnesses, and what enclave borrows

We surveyed three existing AI coding/agent harnesses — **Cursor**, **opencode**,
and **pi** — to learn from their architectures before committing to enclave's.
This records what we found and what we adopt.

The headline conclusion: **none of them solve enclave's actual problem.** They
all assume *the model is trusted and sending data to it is fine*. enclave
inverts that trust boundary. So we copy **mechanics, not data-flow** — which is
also why enclave is a new repo, not a fork of any of these.
...
- **Local OpenAI-compatible models are first-class** via
  ...
  This is exactly enclave's `inference-server` seam, already proven.
...
- **"No in-process security" thesis:** once an agent can read + execute +
  ...
  (container with `--network none`, read-only mounts, tmpfs). This *is* enclave's
  containment model.
...
## What each validates about enclave's design
...
## Decision: what enclave adopts

**Base on pi's philosophy** — it's the closest fit to enclave's
...
exact piece enclave's code-runner depends on. Take Seatbelt / Landlock+seccomp /
...
local-model orchestrator) drive the harness. Worth it only when enclave needs to
...
**Reject from all three: open egress paths.** enclave must **default-deny every
