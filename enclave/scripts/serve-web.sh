#!/bin/bash
#
# Start the enclave web server on 127.0.0.1:10000 and open the chat UI. If
...
#
# Run from the enclave install dir with the toolchain env set (CONDA_PREFIX,
# MODULAR_HOME, PATH). Used by `pixi run serve-web` and `millfolio enclave web`.
set -u
PORT=10000

if [ ! -x ./build/enclave-server ]; then
    echo "enclave web server not built — run: millfolio enclave install" >&2
    exit 1
fi

# The enclave web server is VAULT-ONLY: /chat answers questions about the vault
# dir (ENCLAVE_VAULT_DIR, else $MILLFOLIO_VAULT / $ENCLAVE_DATA / ~/millfolio). The
...
# Export them so the enclave-server process env propagates to the sandboxed
# generated program (which inherits the parent's environment) over loopback.
# (The server is unconditionally vault-only — no ENCLAVE_VAULT gate.)
export ENCLAVE_VAULT_DIR="${ENCLAVE_VAULT_DIR:-}"
export MILLFOLIO_VAULT="${MILLFOLIO_VAULT:-$ENCLAVE_VAULT_DIR}"
...
echo "mode:     VAULT (dir: ${ENCLAVE_VAULT_DIR:-<resolved>})" >&2
