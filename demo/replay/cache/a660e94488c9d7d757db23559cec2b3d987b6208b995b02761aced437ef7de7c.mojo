...
def main() raises:
    ...
    if not have:
        for i in range(len(files)):
            ...
            var ans = ask_local_batch(
                "Use ONLY the text provided. If it contains a transaction or purchase with an amount, "
                "reply as 'MERCHANT | AMOUNT | DATE' (e.g. 'Corner Store | 42.10 | 2024-03-15'). "
    ...
    if have:
        var msg = String("Your biggest transaction was ") + money(top_amount)
        if top_merchant != "":
            msg = msg + " at " + top_merchant
        if top_date != "":
            msg = msg + " on " + top_date
        if top_direction != "":
            msg = msg + " (" + top_direction + ")"
        msg = msg + "."
        print_answer(msg)
    else:
