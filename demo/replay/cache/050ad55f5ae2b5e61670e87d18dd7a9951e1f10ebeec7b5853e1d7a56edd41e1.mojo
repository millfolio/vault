from vault import *
def main() raises:
    var hits = search("vehicle registration license plate car", 10)
    for i in range(len(hits)):
        ref c = hits[i]
        var p = ask_local("Use ONLY the text provided. If it contains a license plate number, reply with just the license plate number. Otherwise reply exactly 'none'. Do not guess or invent.", c.text)
        var ps = String(p.strip())
        if ps != "none" and ps != "":
            print_answer("Your license plate number is " + ps + ".")
            return
    # Fallback: scan all files directly
    var files = manifest()
    for i in range(len(files)):
        progress("scanning " + files[i].alias + " for license plate")
        if files[i].kind == "pdf":
            var text = pdf_text(files[i].alias)
            var p = ask_local("Use ONLY the text provided. If it contains a license plate number, reply with just the license plate number. Otherwise reply exactly 'none'. Do not guess or invent.", text)
            var ps = String(p.strip())
            if ps != "none" and ps != "":
                print_answer("Your license plate number is " + ps + ".")
                return
        elif files[i].kind == "docx":
            var text = docx_text(files[i].alias)
            var p = ask_local("Use ONLY the text provided. If it contains a license plate number, reply with just the license plate number. Otherwise reply exactly 'none'. Do not guess or invent.", text)
            var ps = String(p.strip())
            if ps != "none" and ps != "":
                print_answer("Your license plate number is " + ps + ".")
                return
        elif files[i].kind == "md":
            var text = md_text(files[i].alias)
            var p = ask_local("Use ONLY the text provided. If it contains a license plate number, reply with just the license plate number. Otherwise reply exactly 'none'. Do not guess or invent.", text)
            var ps = String(p.strip())
            if ps != "none" and ps != "":
                print_answer("Your license plate number is " + ps + ".")
                return
    print_answer("I couldn't find a license plate number in your vault.")