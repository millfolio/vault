...
from vault.amounts import parse_amount, format_money
...
def _fm(got: String, want: String, label: String) -> Bool:
    var k = (got == want)
    print("[" + ("PASS" if k else "FAIL") + "] " + label + " -> " + got + " (want " + want + ")")
    return k
...
def main() raises:
    ...
    ok = _fm(format_money(31241.059999999998), "$31,241.06", "ugly float rounds to cents") and ok
    ok = _fm(format_money(61147.01), "$61,147.01", "thousands separator") and ok
    ok = _fm(format_money(1234567.5), "$1,234,567.50", "millions grouped") and ok
