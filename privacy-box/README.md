...
Rather than ship the data to the frontier model, privacy_box flips the
...
privacy_box v1 defends against an **honest-but-careless** remote provider — one
...
- **Confidentiality** (owned by privacy_box): nothing sent to the remote model
...
- **Mechanism vs. policy.** `inference-server` (local inference) and the sandbox
  (containment) are mechanism. privacy_box is confidentiality _policy_ on top.
...
- **Sanitize the schema, not just the values.** Column/table names can leak on
  their own; privacy_box aliases them, mapping real names back locally.
...
src/privacy_box.mojo + src/server.mojo   composition root: CLI vault harness + HTTP server
web/                                  privacy_box for the web — local React chat UI
```

## Configuration

privacy_box reads `~/.config/privacy_box/config.json` (override the path with
`PRIVACY_BOX_CONFIG`). It's parsed with the `json` fork (`src/settings.mojo`). All
...
| `use_local_summary`   | `false`                        | `PRIVACY_BOX_LOCAL` (set = true)                     |
