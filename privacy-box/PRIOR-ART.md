# Prior art: agent harnesses, and what privacy_box borrows

We surveyed three existing AI coding/agent harnesses — **Cursor**, **opencode**,
and **pi** — to learn from their architectures before committing to privacy_box's.
This records what we found and what we adopt.

The headline conclusion: **none of them solve privacy_box's actual problem.** They
all assume *the model is trusted and sending data to it is fine*. privacy_box
inverts that trust boundary. So we copy **mechanics, not data-flow** — which is
also why privacy_box is a new repo, not a fork of any of these.
...
- **Local OpenAI-compatible models are first-class** via
  ...
  This is exactly privacy_box's `inference-server` seam, already proven.
...
- **"No in-process security" thesis:** once an agent can read + execute +
  ...
  (container with `--network none`, read-only mounts, tmpfs). This *is* privacy_box's
  containment model.
...
## What each validates about privacy_box's design
...
## Decision: what privacy_box adopts

**Base on pi's philosophy** — it's the closest fit to privacy_box's
...
exact piece privacy_box's code-runner depends on. Take Seatbelt / Landlock+seccomp /
...
local-model orchestrator) drive the harness. Worth it only when privacy_box needs to
...
**Reject from all three: open egress paths.** privacy_box must **default-deny every
