"""settings — load privacy_box config from ~/.config/privacy_box/config.json + env.
...
def load_config() -> Config:
    ...
    local_url = _env_or("PRIVACY_BOX_LOCAL_URL", local_url^)
    local_model = _env_or("PRIVACY_BOX_LOCAL_MODEL", local_model^)
    ...
    data_dir = _env_or("PRIVACY_BOX_DATA", data_dir^)

    # empty data_dir -> the default under ~/.config/privacy_box (created/asked for at
    # first run by privacy_box.mojo). Kept here so the key is overridable everywhere.
    if data_dir == "":
        data_dir = getenv("HOME", "") + "/.config/privacy_box/data"
