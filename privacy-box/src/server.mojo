"""server — privacy_box over HTTP (flare). The local backend for privacy_box-web (web/).
...
in a heap `PrivacyBoxState` reached through a pointer so the borrowed-self handler
...
struct PrivacyBoxState(Movable):
...
struct Api(Handler, Copyable, Movable):
    ...
    def serve(self, req: Request) raises -> Response:
        ...
        if path == "/health":
            return _cors(ok("privacy_box ok"))
...
def main() raises:
    ...
    # (PRIVACY_BOX_VAULT_DIR / $MILLFOLIO_VAULT / $PRIVACY_BOX_DATA / ~/millfolio) and route
    ...
    var st = PrivacyBoxState(orch^, vault_dir^)
    var sp = alloc[PrivacyBoxState](1)
    sp.init_pointee_move(st^)
    var api = Api(sp)

    print("privacy_box serving on http://127.0.0.1:", PORT, "  (flare)", sep="")
