#!/usr/bin/env bash
# privacy_box VAULT sandbox spike — prove the loopback-only network rule on macOS.
#
# Renders sandbox/privacy_box-vault.sb.template and verifies the ONE thing that
...
# writes, $HOME deny) is identical to privacy_box.sb.template and already proven by
...
TEMPLATE="$HERE/privacy_box-vault.sb.template"
ROOT="${TMPDIR:-/tmp}/privacy_box_vault_spike.$$"
...
PROFILE="$ROOT/privacy_box-vault.sb"
...
echo "privacy_box VAULT sandbox spike  (profile: $PROFILE)"
