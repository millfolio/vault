#!/usr/bin/env bash
#
# Release privacy_box at the current HEAD: push main, create + push an annotated
# tag, and wait for the "privacy_box zip" CI (which builds & attaches privacy_box.zip).
#
...
echo "==> waiting for 'privacy_box zip' CI…"
sleep 6
RID="$(gh run list -R "$REPO" --workflow 'privacy_box zip' -L1 --json databaseId -q '.[0].databaseId')"
