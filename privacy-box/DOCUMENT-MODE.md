# privacy_box document mode — design

**Goal: let users drop a folder of their own files — PDFs, Word docs, text,
emails — and ask questions about them, with the same guarantee privacy_box gives
today: a frontier model writes the code, your content never leaves the machine.**

Today privacy_box v1 is built end to end around a **single CSV table**
...
privacy_box's confidentiality trick (README): the model is allowed to see the
...
- `wiring.mojo` / `privacy_box.mojo` / `server.mojo`: `has_csv`, `_resolve_data_dir`,
  `seed_demo`.
...
Documents tempt a task privacy_box **cannot** safely serve via the remote model:
...
privacy_box should detect/declare this boundary and route accordingly, rather than
...
Output: a normalized text cache (e.g. `~/.config/privacy_box/cache/<run>/doc_0.txt`)
